module.exports = {
  ...
  body: (p) => {
    ...
    const get_traces = () => {
      ...
      // Only generate traces for P0/P1/P2 (i=0,1,2) and their mirrors V5/GND/V3.
      // The remaining pins are left untraced and must be jumpered manually.
      for (let i = 0; i < 3; i++) {
        ...
        // On the last row the downward arc would clash with the bottom pad ovals,
        // so we route straight across at the same Y instead.
        if (i < spacing.total_pin_num / 2 - 1) {
          ...
          // Last row: route straight across at pin Y to avoid the bottom pad ovals
          out += `\t(segment (start ${adjust_point(rx - spacing.pin_to_female_pad, y)}) `
               + `(end ${adjust_point(lx + spacing.pin_to_via, y)}) (width 0.25) (layer "F.Cu"))\n`;
        }
